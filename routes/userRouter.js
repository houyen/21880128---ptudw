...
router.post("/register", (req, res, next) => {
  ...
  let keepLoggedIn = req.body.keepLoggedIn != undefined;
  // Kiem tra confirmPassword va password giong nhau
  ...
  userController
    ...
    .then((user) => {
      if (user) {
        return res.render("register", {
          message: `Email ${email} exists! Please add another email address.`,
          type: "alert-danger",
        });
      }
      // Tao tai khoan
      ...
      return userController.createUser(user).then((user) => {
        if (keepLoggedIn) {
